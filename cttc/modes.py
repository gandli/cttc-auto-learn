...
from cttc.ui import ask_new_target
...
async def enforce_single_tab(page: object, log: Logger) -> None:
    """关闭所有多余标签页，只保留当前页面
...
async def _play_course_with_retry(
    page: object,
    course_url: str,
    course_title: str,
    course_id: str,
    ...
    # 播放视频
    player = VideoPlayer(page, config, log, progress, status)
    await player.setup()
...
async def mode_hours(
    ...
    try:
        while True:
            ...
            # 获取优先课程（专题/任务中的课程）
            priority_course_ids: set[str] = set()
            ...
            for c in my_courses:
                ...
                # 判断是否为优先课程
                is_priority = any(
                    pid and c["title"][:20].startswith(pid)
            ...
            for course in pending:
                ...
                success = await _play_course_with_retry(
                    client.page, video_url, course["title"], course_id,
                    config, log, progress, status, monitor
                )
...
async def mode_courses(
    ...
    for i, course in enumerate(pending, 1):
        ...
        success = await _play_course_with_retry(
            client.page, course["url"], course["title"], course_id,
            config, log, progress, status, monitor
        )
